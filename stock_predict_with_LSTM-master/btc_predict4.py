...
trainnumlength = 30
df=pd.read_csv('traindata9128_30.csv',header=None)     #读入股票数据
data=df.iloc[:,0:trainnumlength+2].values  #取第1-20列 训练数据
...
NUM_EPOCHS = 10
model = Sequential()
model.add(Embedding(8500, EMBEDDING_SIZE,input_length=trainnumlength))
